...
export const LoansView: React.FC<{
  ...
  const filteredLoans = loans.filter(l => {
    const isSettled = l.status === 'Paid' || l.status === 'Settled' || (l.amountRepaid || 0) >= l.amount;
    if (activeFilter === 'lent') return l.type === 'lent' && !isSettled;
    if (activeFilter === 'borrowed') return l.type === 'borrowed' && !isSettled;
    ...
    <div className="space-y-6">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-bold text-sm text-slate-900">{loan.person}</h4>
                          <span
                  ...
                  <div className="my-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                    <div className="flex justify-between items-baseline mb-1">
                      ...
                      <span className="text-sm font-bold text-slate-900">{formatCurrency(loan.amount, loan.currency)}</span>
                    </div>
                      ...
                      <div className="pt-2 border-t border-slate-100">
                        ...
                        <div className="space-y-1">
                          {loan.repayments.map(r => (
                            <div key={r.id} className="flex justify-between text-[11px] text-slate-600">
      ...
      {repayingLoan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4">
          <form onSubmit={handleRepaySubmit} className="bg-white border border-slate-200 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Deposit / Pay From</label>
                <select
