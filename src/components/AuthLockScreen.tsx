...
export const AuthLockScreen: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4">
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-2xl p-8 text-center text-slate-900">
        <div className="w-16 h-16 bg-emerald-100 border border-emerald-300 text-emerald-600 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-sm">
          <Lock className="w-8 h-8" />
        </div>

        <h2 className="text-2xl font-bold tracking-tight">Personal Finance Locked</h2>
        <p className="text-sm text-slate-500 mt-1 mb-6">
          Enter PIN for <span className="font-semibold text-slate-700">{profile.name}</span>
        </p>
        ...
        <div className="grid grid-cols-3 gap-3 max-w-xs mx-auto mb-6">
          {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map(num => (
            <button
              ...
              className="h-14 rounded-xl bg-slate-100 hover:bg-slate-200 text-lg font-bold text-slate-800 transition active:scale-95 border border-slate-200/50"
            >
              {num}
            </button>
          ))}
          <button
            ...
            className="h-14 rounded-xl text-xs font-semibold text-slate-500 hover:text-emerald-600 transition"
          >
            Forgot?
          </button>
          <button
          ...
          <button
            ...
            className="h-14 rounded-xl bg-slate-100 hover:bg-slate-200 text-xs font-semibold text-slate-700 transition active:scale-95"
          >
            Delete
          </button>
        </div>
        ...
        <p className="text-xs text-slate-400">
          Default test PIN is <code className="bg-slate-100 px-1.5 py-0.5 rounded font-mono font-bold text-emerald-600">1234</code>
        </p>

        {showForgot && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <form onSubmit={handleResetPin} className="bg-white p-6 rounded-2xl max-w-sm w-full border border-slate-200 text-left">
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 bg-white text-slate-900 mb-4 focus:ring-2 focus:ring-emerald-500 outline-none"
              />
